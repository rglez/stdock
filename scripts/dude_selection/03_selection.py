...
topo_traj_pick = join(root_dir, 'scripts/dude_selection/topo_traj.pick')
clusters_dir = join(root_dir, 'scripts/dude_selection/02_clustering')
out_dir = join(root_dir, 'scripts/dude_selection/03_selection')
...
for case in by_nrots:
    mol_suppl = [Chem.AddHs(Chem.MolFromPDBFile(x['top']), addCoords=True) for
                 x in by_nrots[case]]
