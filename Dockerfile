...
# and `pip install stdock` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "stdock", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n stdock -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "stdock", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "stdock", "YOUR_CONSOLE_SCRIPT"]
